...
module.exports = async client => {
    (await glob(`${process.cwd()}/events/*/*.js`)).map(async eventFile => {
        ...
        if (!eventsList.includes(event.name) || !event.name) {
            return console.log(`------\n${client.timestampParser()} --> Evenement non-déclenchée: erreur de typo (ou pas de nom)\nFichier --> ${eventFile}`);
        }
